...
function usePaginator({
  ...
  const getPages = useCallback(() => {
    ...
    if (currentPage > onEachSide) {
      // to avoid negative pages
      for (let i = onEachSide; i >= 0; i--) {
        // >= ensures the current page is pushed
  ...
  return {
    ...
    pages,
    currentPage,
    showPaginator: Math.ceil(count / limit) > 1,
  };
}
...
export default function Paginator({
  ...
  const { from, to, count, pages, currentPage, showPaginator } =
    usePaginator(rest);
  ...
  const PageLimitOptions = () => {
    ...
    return (
      <div className="float-md-end">
        <label htmlFor="limit">Baris:</label>
        <select
          name="limit"
  ...
  return (
    <div className="mb-3">
      ...
      <div className="text-lg-start">
        Tampilkan {from} ke {to} dari {count} baris
      </div>
    </div>
  );
}
