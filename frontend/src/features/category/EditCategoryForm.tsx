...
export const EditCategoryForm = ({ match }: RouteComponentProps<TParams>) => {
  ...
  const handleDestroy = useCallback(async () => {
    if (categoryId.length) {
      try {
        const { message, error, invalidData } = await destroyCategory(
          categoryId
        ).unwrap();
        if (message) {
          history.push({
  ...
  const form = (
    <Formik
      ...
      onSubmit={async (values, actions) => {
        ...
        try {
          const { category, error, invalidData } = await updateCategory(
            values
          ).unwrap();
          ...
          if (invalidData) {
            ...
            setMessage({
              type: 'danger',
              message: 'Terjadi kesalahan, coba cek kembali!',
            });
          }
        } catch (error) {
          setMessage({ type: 'danger', message: error.message });
  ...
  return (
    <>
      ...
      <Modal
        ...
        body="Apakah Anda yakin ingin menghapus kategori? Action ini tidak bisa di undo."
