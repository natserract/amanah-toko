...
async function create(req: Request, res: Response) {
  try {
    ...
    switch (source) {
      ...
      default:
        return res.status(400).json({
          error: `Source must be either 'store'`,
        });
    }
    ...
    return await sequelize.transaction(async (t) => {
      ...
      const transfer = await Transfer.create(
        { productId, quantity, source, destination },
        { transaction: t }
      );
...
async function destroy(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (affectedRows !== id.length) {
      ...
      return res.status(400).json({
        error: `${
          notDeleted > 1 ? `${notDeleted} transfers` : 'Transfer'
        } not deleted. Please try again`,
      });
    } else {
      return res.status(200).json({
        message: `${
          id.length > 1 ? `${id.length} transfers` : 'Transfer'
